...
from src.musetalk.utils.utils import datagen, load_all_model, video2imgs, osmakedirs
from src.musetalk.utils.preprocessing import get_landmark_and_bbox,read_imgs
from src.musetalk.utils.blending import get_image_prepare_material,get_image_blending
...
@torch.no_grad() 
class Muse_Talk:
    def __init__(self, avatar_list = [('Avatar4',6) ], batch_size = 8, fps = 25):
        self.fps = fps
    ...
    def prepare_material(self, avatar_name, bbox_shift = 0):
        ...
        logger.info(f"[Preprocessing] Creating avator: {avatar_name}")
        ...
        logger.info(f"[Preprocessing] Extracting landmarks")
    ...
    def load_material(self, avatar_name):
        ...
        logger.info("[Preprocessing] Reading mask images")
    ...
    def warm_up(self):
        ...
        self.infer(tmp_project_path, audio_path, avatar_name = 'Avatar4')
    ...
    def infer(self, project_path, audio_path, avatar_name):
        ...
        logger.info(f"[THG] Start inferencing Video {video_idx}")
        ...
        logger.info(f"[THG] Processing audio:{audio_path} costs {(time.time() - start_time) * 1000}ms")
        ...
        logger.info(f"[THG] Video {video_idx}: Total process time of {video_num} frames including saving images = {time.time()-start_time}s")
